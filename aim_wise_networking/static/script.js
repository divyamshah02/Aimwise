...
(function themeBoot(){
  ...
  const saved = localStorage.getItem('aimwise-theme');
...
$('#themeToggle')?.addEventListener('change', (e)=>{
  ...
  localStorage.setItem('aimwise-theme', dark ? 'dark' : 'light');
});
...
['a','button','.btn','input','textarea','select','label'].forEach(sel => {
  $$(sel).forEach(el => {
    ...
    el.addEventListener('mouseleave', ()=> ring.style.transform = `translate(${ringPos.x-18}px, ${ringPos.y-18}px)`);
  });
});
